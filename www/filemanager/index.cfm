...
<cfinvoke component="#cffm#" method="init">
	...
	<cfinvokeargument name="includeDir" value="#request.speck.appInstallRoot#">
	<!--- includeDirWeb = web path to the directory specified above. --->
	<cfinvokeargument name="includeDirWeb" value="#request.speck.appWebRoot#">
	...
	<cfinvokeargument name="enableImageDimensionsInDirList" value="false">
...
<cfif listFind("edit,viewsource,viewzip,renameForm,copymoveForm,manipulateForm",action) gt 0>
	<cfoutput><h1>Working with #cffm.getPathType(cffm.createServerPath(variables.subdir,variables.editFilename))#:  #variables.workingDirectoryWeb#/#editFilename# </h1></cfoutput>
<cfelse>
	<cfoutput><h1>#cffm.resourceKit.Msg.t1#:  #variables.workingDirectoryWeb#</h1></cfoutput>
</cfif>
<!--- <cfset variables.listAllFiles = cffm.directoryList(cffm.includeDir,"true")> --->
<cfif variables.action eq "list">
	<!--- do not recurse when listing because it kills the server --->
	<cfset variables.listAllFiles = cffm.directoryList(cffm.includeDir,"false")>
<cfelse>
	<cfset variables.listAllFiles = cffm.directoryList(cffm.includeDir,"true")>
</cfif>
...
<cfif isDefined("variables.errorMessage") and variables.errorMessage neq "">
	<cfoutput>
	<div class="cffm_errorMessage">
		<!--- <h3>#cffm.resourceKit.Msg.t3#</h3> --->
		<p>
		Sorry, the action could not be completed due to the following issues... 
		</p>
		<ul>#variables.errorMessage#</ul>
	</div>
...
<cfif variables.action eq "viewSource" or variables.action eq "edit">
	...
	<textarea class="cffm_editor" id="cffm_edit_area" name="editFileContent"><cfoutput>#variables.content#</cfoutput></textarea>
	<p>
	<cfsavecontent variable="addToHtmlHead">
		<!--- TODO: clean this up, use a structure mapping extensions to syntax files or something --->
		<cfset fileExt = listLast(variables.editFilename,".")>
		<cfif listFindNoCase("cfm,cfc",fileExt)>
			<cfset syntax = "coldfusion">
		<cfelseif fileExt eq "rb">
			<cfset syntax = "ruby">
		<cfelseif fileExt eq "py">
			<cfset syntax = "python">
		<cfelseif fileExt eq "pl">
			<cfset syntax = "perl">
		<cfelseif listFindNoCase("htm,html,rhtml,erb,jsp,asp,aspx",fileExt)>
			<cfset syntax = "html">
		<cfelse>
...
<input type="text" size="20" style="width:130px;" name="createNewFilename" onFocus="select();">
...
<cfif cffm.allowCreateDirectory><input type="submit" class="button" value="<cfoutput>#cffm.resourceKit.buttonText.t8#</cfoutput>" onClick="document.frmCreateNew.createNewFileType.value='directory';"></cfif>
</fieldset>
</td>
</form>
</tr></table>
...
if (listLen(name,".") gt 1) {
	...
	if (listFind(cffm.editableExtensions, variables.extension) gt 0 and type eq "file") {
		...
		variables.viewSource = 1;
		variables.fileIcon = "documenticon.gif";
	} else if (listFind("zip",variables.extension) gt 0) {
		variables.zipFile = 1;
	// new code to allow code files that aren't editable, but for which we can do some syntax hightlighting be viewed in the editor (but not saved)
	} else if (listFind("php,js,pl,cfm,cfc,htm,html,rb,rhtml,erb,jsp,asp,aspx",variables.extension) gt 0) {
		variables.viewSource = 1;
		variables.fileIcon = "documenticon.gif";
	}
} else {
